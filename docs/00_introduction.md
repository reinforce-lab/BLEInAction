...
身の回りにあるあらゆる物が常にネットワークと接続している世界は、モバイルに続く大きな成長を社会にもたらします。ハードウェアとソフトウェアそして通信が1つのサービスとして統合されて、我々のリアルな世界の人と物そしてネットワーク世界のデータとその処理とが密接に連携しあうことで、新たな価値が生み出されるのです。

本書がとりあげる超低消費電力無線通信技術 Bluetooth Low Energy (ブルートゥース ロー エナージー) は、ものをネットワークにつなぐ重要な無線通信技術の1つです。

Bluetooth Low Energy は、2010年に規格制定されたBluetooth4.0の一部です。iPhone が2011年に Bluetooth4.0 に対応したことで、この登場して間もない新しい無線通信技術が、いま急激な勢いで普及しています。Bluetooth Low Energy を採用したiOSデバイスと連携するユニークな製品やサービスを発表するベンチャー企業が、続々と登場しています。

Bluetooth Low Energy はiOSアプリケーションとつながるハードウェアを生み出しました。このハードウェアは、アプリケーション(Application)とアクセサリ(Accessory)を組み合わせた、アプセサリ(Appcessory = App + cessory) という造語で呼ばれます。iOSアプリケーションとハードウェアが密接に連携して生まれる新しい魅力は、従来のアクセサリという単語では、表現できません。ですから、アプセサリという、新しい概念を表すための造語が作られました。

このように、Bluetooth Low Energy には、従来あった"もの"の見方や価値あるいは意味を変えてしまう力があります。

アプセサリは、人とものとネットワークが連携する世界の、1つの小さな領域にすぎません。Bluetooth Low Energyがもたらす価値は、ハードウェア単体にはありません。その価値は、人とものとネットワークの統合と連携からもたらされます。

この新しい時代に自分たちの一歩を踏み出すには、Bluetooth Low Energy の知識が不可欠です。この知識は、エンジニアやプログラマが知っていればいいだけの知識ではありません。むしろ、新しい時代のビジネス・モデルを構築する経営者や企画を担う人たちにこそ、その知識が必要なのです。

そして一歩一歩を歩み続けるには、チームの力が必要です。そこには、事業計画と企画、そしてハードウェア、無線通信技術、iOSアプリケーションとネットワーク・サービス開発の密な連携が欠かせません。つまり、チームのメンバー、経営や企画者、iOSアプリケーション、電気回路やマイクロプロセッサのファームウェア開発者、の連携が重要なのです。

その連携の礎となるのが、Bluetooth Low Energy の知識です。本書は、Bluetooth Low Energyとは何か、またそれがどのようなユーザ体験をもたらすのかを述べます。次に、ユーザ体験を作るうえで欠かせない、無線通信技術とアプリケーション開発およびハードウェア試作を、実践的かつ実用的に解説します。
