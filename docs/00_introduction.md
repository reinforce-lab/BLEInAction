...
身の回りにあるあらゆるものがネットワークとつながる時代、そのつなぐ技術の1つが、超低消費電力無線通信技術 Bluetooth Low Energy (ブルートゥース ロー エナージー) です。

新しい無線通信技術の普及には、その通信相手の普及が必要です。Apple社が2011年に世界で初めて Bluetooth Low Energy に対応した iPhone4s を発売したことで、この壁は打ち破られました。それ以来、Bluetooth Low Energy を採用したiOSデバイスと連携するユニークな製品やサービスを発表するベンチャー企業が、続々と登場しています。


思い返せば、2000年頃にインターネットという単語が、世界を変えると大きな話題になりました。ADSLモデムが駅前で配布され、高速なネットワークが急速に一般家庭にまで普及しました。この14年間で、インターネットという単語は、社会の大きな変化をもたらしました。

その変化は、今も続いています。人々が見る画面はテレビからパソコンへ、さらに常にネットワークに接続しているスマートフォンへと移り変わりました。音声とテキストそして決済も統合されたコミュニケーション・アプリケーション、そしてソーシャル・メディアの登場が、コミュニケーションの形を変化させました。スマートフォンを通じてすべてが繋がる、成熟したモバイル市場が登場したのです。

次の大きな成長は、身の回りにあるあらゆる物が常にネットワークと接続することから、はじまります。ハードウェアとソフトウェアそして通信が1つのサービスに統合されて、我々のリアルな世界の人と物、そしてネットワーク世界のデータとデータ処理とが、さらに密接に連携しあうことで、新たな価値が生み出される時代です。

この時代には、Bluetooth Low Energy の知識は、エンジニアやプログラマだけが知っていればいい知識ではありません。2000年台のインターネットがもたらした社会の変化でそうであったように、むしろ、経営や企画を担当する人にこそ、その技術の知識が必要なのです。

Bluetooth Low Energy は、いままであったものの意味を変えてしまいます。例えば、Bluetooth Low Energy を通じてiOSデバイスとつながるデバイスは、アプリケーション(Application)とアクセサリ(Accessory)を組み合わせたアプセサリ(Appcessory = App + cessory) という造語で呼ばれます。

iOSアプリケーションとハードウェアが密接に連携して生まれる新しい魅力は、従来のアクセサリという単語では、表現できません。ですから、アプセサリという、新しい概念を表すための造語が作られています。

このアプリせサリを生み出すには、事業計画と企画、そしてハードウェア、無線通信技術、iOSアプリケーションとネットワーク・サービス開発の密な連携が不可欠です。つまり、チームのメンバー、経営や企画者、iOSアプリケーション、電気回路やマイクロプロセッサのファームウェア開発者、の連携が重要なのです。

本書は、Bluetooth Low Energy とは何なのか、そのユーザ体験はなにかを、、ユーザ体験を作るうえで欠かせない無線通信技術とアプリケーション開発およびハードウェア試作までを、実践的かつ実用的に解説します。
