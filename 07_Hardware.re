= ものへのBluetooth Low Energy技術の取り込み
...
#@# 内部のソフトウェア、システム開発者とのやり取り。
#@# 	GATT基盤、HomeKit的、Zigbeeとか。
#@# 	ソフトウェアがどこで実行されるのか、
#@# 	安全性検証、
#@# 内部のハードウェア、電気回路とのやり取り。
#@# 	パネル設計と同じなので。後からつけた場合は、電源まわり。常時稼働? 動作環境?	
#@# 	電磁両立性、電波法認証
#@# 説明書や商品ラインナップの人たちと
#@# 	販売する国の確認、認証の表示。
#@# 	製品の認証の取り方。
#@# 外部のソフトウェア、システムとのやり取り。
#@# 	プロファイルはカスタム、標準、それともHomeKit的なもの
#@# 	開発途中のチェックポイントの確認、SDKは提供するのか、提供されるのか、	
...
#@# とても高性能なプロセッサを使うもの -> 無線通信部分は別搭載。HCIで接続。
#@# とても安価で大量に製造するもの -> 1チップ構成。無線機能+アプリも実行処理
#@# 	上記の場合で、演算処理などアプリ実行で余裕を持たせたい場合	-> 2チップ構成
...
#@#GATT基盤、ソフトウェアのスタックがどこにあるのかを書くこと
2チップ構成は、高性能なプロセッサの周辺部品として無線通信の半導体が配置される構成です。パーソナル・コンピュータやスマートホン、あるいは組み込みLinuxボードなどは、この構成を取ります。


== 開発体制とチェックリスト

何をするか		実験			試作,組織内利用	 事業,一般向け
----------------------------------------------------------------------------
利用範囲	   自分で利用	　  仲間内利用		 一般販売
開発時間	 	1時間		   1ヶ月		   1年
数量、1個		10~100	　　   ~1000			~1万個単位,継続した製造
製造			手作り		  外部委託			 製造専門に依頼
電波法			対応		   対応				 対応
SIGへの製品登録　不要			営利では必要		必要
廃棄			粗大ゴミ	   産業廃棄物		地方自治体
...
#@# 段階
#@#----------------------------------------------------------------------------
#@# 調査/実験	 新たな利用場面や体験を考える
#@# 試作		限定した機能の何かを作る
#@# 設計		見ず知らずの誰かが長期間使用できる、製造可能なデータを作る、
#@# 製造管理	指定された製造品質を見たし、性能を満たしているものを多数製造する


#@# 1章で関わり方を見てきた。個人や学習、事業化の中でのチームの一員として。
...
#@# 成果物、修めるもの、
#@# 学習であれば、教材でマイルストーンがある、それで動かしたりして。自分の試行錯誤、経験。
#@# 個人の興味、何かが動く何かを作って、のぞみのものを作る。そこで完了かもしれないし、作り方や動画を公開したり、共有できる形にして残すのかもしれない。再現性があればいいくらい。
#@# 事業あるいはチームの一員であれば、製造可能な設計にして、大量に製造すること、システムの一部として通信だからBLEが入っている。

#@# チェックポイント:
#@# - 他の無線と混信しないか。WiFiとか。
#@# - アンテナ部分を金属で覆ったりしていないか。
#@# - 要求と構成がチグハグ、例えば電源ONにするのに、メイン電源は物理スイッチだったりしないか
#@# - 適切な無線通信なのか? 他の方がいいのではないのか?
