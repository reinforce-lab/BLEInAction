...
//	const unsigned int VendorID = 0;//0x04d8; //wiimote
//	const unsigned int ProductID =0;//0x003f;
//	VendorID = 0;//0x04d8; //wiimote
//	ProductID =0;//0x003f;
...
bool CUsbhidioc::ReadReport(unsigned char InBuffer[])
{
	...
	*/
		...
		(ReadHandle,
		InputReport,
		20, //Capabilities.InputReportByteLength,     08Mar2005 - Capabilities call fails on WinXP
		&NumberOfBytesRead,
		...
		*/
	    ...
	    ResetEvent(hEventObject);
            ...
            unsigned int i;
            for (i=0; i < 18 /*Capabilities.InputReportByteLength*/; ++i)
...
bool CUsbhidioc::WriteReport(unsigned char OutputBuffer[], unsigned int nBytes)
{
    ...
    ULONG   Result;
    char    OutputReport [100];
    unsigned int     i;

	//The first byte is the report number.
    OutputReport[0]=0;//0x18;
    for (i=0; i < nBytes; i++)
        OutputReport [i+1] = OutputBuffer[i];
    for (i = nBytes+1; i < 18; ++i)
        OutputReport [i] = '0';  // Pad buffer with Null Commands
...
/*
	...
	*/
    ...
    if (DeviceDetected)
    {
		...
		(DeviceHandle,
		OutputReport,
        nBytes+1, //Capabilities.OutputReportByteLength,  08Mar2005 - Capabilities call fails on WinXP
		&BytesWritten,
		NULL);
