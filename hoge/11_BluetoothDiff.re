...
ステートとロールの組み合わせの説明が、許された状態とロールの組み合わせと変更されている。

デバイスのアドレスのタイプ
Public, Random Static、(v5.2) RResolvable Private address

Air Linkのパケット。
1Msと2Msでプリアンブルが異なる。パケット途中でシンボルレートが切り替わることはない。
PDUは2~258oct。ver4.2だと~257だったけど。
トーンが末尾に追加。

アクセスアドレス
ver4.2だとアドバタイジングは決まった値、リンクはランダム生成だったけど、複雑化。
Periodic Advだと、トレインを生成するたびに、新たなアドレスを生成する。

接続してても、Connected Isochronous Streamで、新しいアクセスアドレスを生成する。
ブロードキャストのIsoなアドレスは、SAAから生成で、SAAは、ビット配列にルールがあって。

PDUの種類が増えてる。
アドバチ人る、データ PhysicalChannePDU、IsochronousPhysica lChannePDU。

トーン信号は、ホワイトニングしない、一定の1での変調を与えたもの。
Isoにはつかない。

LE Coded PHYでの、パケット構造は別に定義。

LL COntrol PDU、~0x15までだったのが、めっちゃ拡張されている。
LL_PHY_REQ, ...
